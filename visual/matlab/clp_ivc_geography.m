function hdl = clp_ivc_geography(varargin)
...
if ishold(gcf); clf reset;   
 ...
 %cmap=cmap(50:end,:);
  ...
  mira=mira(mira>0);mafg=mafg(mafg>0);mind=mind(mind>0);mtaj=mtaj(mtaj>0);mb=mb(mb>0);
  set([mafg;mind;mira;mtaj;mb],'FaceColor','none');
  m_grid; hold on;

end
  
% Place names
countries = {'Iran','Afghanistan','Pakistan','India','China','Tajikistan'};
rivers = {'Indus','Ghaggar Hakra','Ganges','Rajasthan'};
provinces = {'Punjab','Sindth','Gujarat','Baluchistan','Kashmir'};
features = {'Aravalli range','Kachi plain','Punjab plain','Thar desert',...
    'Makran coast','Deccan plateau','Himalayas'}
...
cl_print('name','ivc_geography','ext','pdf');
