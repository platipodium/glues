function [retdata,basename]=clp_nc_grid_variable(varargin)
...
arguments = {...
  ...
  {'mult',1},...
  {'div',1},...
  {'file','../../eurolbk_base_0.5x0.5.nc'},...%  {'retdata',NaN},...
  {'basename','grid_variable'},...
  {'nocolor',0},...
  {'ncol',19},...
  {'nogrid',0},...
  {'notitle',0},...
...
for varid=0:nvar-1
  ...
  if strcmp(varname,'lat') lat=netcdf.getVar(ncid,varid); latdim=dimids; end
  if strcmp(varname,'lon') lon=netcdf.getVar(ncid,varid); londim=dimids; end
  if strcmp(varname,'time') time=netcdf.getVar(ncid,varid); timedim=dimids; end
  if strcmp(varname,'latitude') lat=netcdf.getVar(ncid,varid); latdim=dimids; end
  if strcmp(varname,'longitude') lon=netcdf.getVar(ncid,varid); londim=dimids; end
end

varname=variables; varid=netcdf.inqVarID(ncid,varname);
[varname,xtype,dimids,natts]=netcdf.inqVar(ncid,varid);
description=varname; units='';
for attid=0:natts-1
  attname = netcdf.inqAttName(ncid,varid,attid);
  if strcmp(attname,'description') description=netcdf.getAtt(ncid,varid,attname); end
  if strcmp(attname,'units') units=netcdf.getAtt(ncid,varid,attname); end
end
...
data=double(netcdf.getVar(ncid,varid,start,count));

if isnumeric(mult) data=data .* mult; 
else
  varid=netcdf.inqVarID(ncid,mult);
  factor=double(netcdf.getVar(ncid,varid,start,count));    
  data = data .* repmat(factor,size(data)./size(factor));
end

if isnumeric(div) data=data ./ div; 
else
  factor=double(netcdf.getVar(ncid,netcdf.inqVarID(ncid,div),start,count));    
  data = data ./ repmat(factor,size(data)./size(factor));
end
...
if isfinite(threshold)
  error('Not implemented yet, see line 103');
  ntime=1;
...
if (marble==1)
  ...
  elseif (marble==2)
    ...
    pm=clp_relief;
  elseif (marble==3)
    [topo,tlon,tlat]=clp_globe('fig',figid,'lon',lonlim,'lat',latlim,'nogrid',nogrid,'nocoast',1);
...
for it=1:ntime
  ...
  if minmax(2)>minmax(1)
    resvar=squeeze(round(((data(:,:,it)-minmax(1)))./(minmax(2)-minmax(1))*(length(cmap)-1))+1);
  else
    resvar=data*0+1;
    ncol=1;
  ...
  p=m_pcolor(lon,lat,resvar');
  shading flat;
  tcmap=demcmap(topo);
  
  error('Implemented up to here');
