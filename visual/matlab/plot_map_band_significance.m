function plot_map_band_significance(varargin)
...
if exist(sigfile,'file') load(sigfile); 
else 
  sig.low=zeros(nsites,3);
  sig.upp=zeros(nsites,3);
  sig.tot=zeros(nsites,3);
  sig.nyquist=zeros(nsites);
  for isite=1:nsites 
    [dummy,sitename,dummy,dummy]=fileparts(holodata.Datafile{isite});
    %fprintf('Site %s ',sitename);
    ilow=0; ired=0; iupp=0;
    for ired=1:nred
      if strcmp(redfitdata.dataset{ired},[sitename '_low']) ilow=ired;
      elseif strcmp(redfitdata.dataset{ired},[sitename '_upp']) iupp=ired;
      elseif strcmp(redfitdata.dataset{ired},[sitename '_tot']) itot=ired;
      end;
    end;
    %fprintf(' %d %d %d\n',ilow,iupp,itot);
    fprintf('.');
    if iupp sig.upp(isite,:)=[redfitdata.sig1(iupp);redfitdata.sig2(iupp);redfitdata.sig3(iupp)]; end  
    if ilow sig.low(isite,:)=[redfitdata.sig1(ilow);redfitdata.sig2(ilow);redfitdata.sig3(ilow)]; end  
    if itot sig.tot(isite,:)=[redfitdata.sig1(itot);redfitdata.sig2(itot);redfitdata.sig3(itot)]; end  
  end;
  save('sig_o4n2w1','sig');
end;
...
cown=[[0 255 255];[0 255 255];[225 255 255];[255 255 255];[255 225 225];[255 0 0];[255 0 0]]./255;
colormap(cown);
...
if 0%~exist('wmap.mat','file') 
  for isite=1:nsites
    % Skip all sites where there is no significance (at 90%) in both lower
    % and upper Holocene
    %if (max([sig.low(isite,:) sig.upp(isite,:)]) < 2) continue; end;
