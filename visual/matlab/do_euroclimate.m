...
if any(doplots==33) 
  ...
  sce='0.4';
  for it=1:nmovtime
    [d,b]=clp_nc_variable('var','farming','reg','lbk','marble',2,'transparency',1,'nocolor',0,...
      'showstat',0,'lim',[0 1],'timelim',movtime(it),...
...
if any(doplots==6) 
   ...
   for ir=1:nreg
    ...
    [data,basename]=clp_nc_variable('var','farming_timing','reg',reg,'marble',0,'transparency',0,'nocolor',0,...
      ...
      'file',strrep(file,'0.4','diff'),'figoffset',0,'sce','diff',...
      'noprint',1,'notitle',1,'ncol',11,'cmap','hotcold');
  ...
  scecolors=jet(11);
   ...
   for isce=[1 5] %length(sces):-1:1
    ...
    if isce==5;
      ...
      xlabel('Simulated transition duration (years)');
      ylabel('Reconstr. transition duration (years)');
      limit=[100 1400];
      valid1=find(isfinite(stranstime(:,1)) & isfinite(transtime(:,1)));
      valid5=find(isfinite(stranstime(:,5)) & isfinite(transtime(:,5)));
      limit1=cl_minmax(transtime(valid1,1));
      limit5=cl_minmax(transtime(valid5,5));
      ...
      p2(3)=plot(transtime(valid5,5),stranstime(valid5,5),'ko','MarkerFaceColor','k');
      
      b1=polyfit(transtime(valid1,1),stranstime(valid1,1),1);
      b2=polyfit(transtime(valid5,5),stranstime(valid5,5),1);
      ...
      [r,p]=corrcoef(transtime(valid5,5),stranstime(valid5,5));
      ...
      cl_print('name','transition_duration','ext','eps');
    end
   ...
   %    & (lon(ireg)<32 | lat(ireg)>38 ) );
    ...
    if isce==5
      ...
      set(gca,'FontSize',15,'FontName','times');
      xlabel('Simulated onset (cal BC)');
      ylabel('Reconstructed onset (cal BC)');
      limit=[-8500 -3500];
      valid1=find(isfinite(timing(:,1)) & isfinite(stiming));
      valid5=find(isfinite(timing(:,5)) & isfinite(stiming));
      limit1=cl_minmax(timing(valid1,1));
      limit5=cl_minmax(timing(valid5,5));
      p2(1)=plot(limit,limit,'k--','color',lg,'LineWidth',2,'visible','on');
      p2(2)=plot(timing(valid1,1),stiming(valid1),'ko','MarkerFaceColor',mg,'Color',mg);
      p2(3)=plot(timing(valid5,5),stiming(valid5),'ko','MarkerFaceColor','k');
      
      b1=polyfit(timing(valid1,1),stiming(valid1),1);
      b2=polyfit(timing(valid5,5),stiming(valid5),1);
      ...
      [r,p]=corrcoef(timing(valid5,5),stiming(valid5));
      fprintf('Onset 5 R^2=%.2f n=%d p=%.3f',r(2,1).^2,length(valid5),p(2,1));
      
      
      valid=find(isfinite(timing(:,5)) & isfinite(stiming) & isfinite(transtime(:,5))...
          & isfinite(stranstime(:,5)));
      valid=valid5;
      for iv=1:-length(valid)
        ir=valid(iv);
        plot(repmat(timing(ir,5),1,2),stiming(ir)+0.5*stranstime(ir,5)*[-1 1],'k-');
        plot(timing(ir,5)+0.5*transtime(ir,5)*[-1 1],repmat(stiming(ir),1,2),'k-');
      end
        
      ilev=find(stiming(valid5)<-7000);
      p2(6)=plot(timing(valid5(ilev),5),stiming(valid5(ilev)),'ko','MarkerFaceColor','r','visible','off');
      uistack(p2([2 3]),'top');
