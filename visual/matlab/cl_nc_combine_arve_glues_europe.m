function cl_nc_combine_arve_glues_europe(varargin)
...
pop=area(aindex(europe),:).*glues.value(aindex(europe),:);
...
%% Single regions of Europe
% By applying the same logic as for all of Europe to individual countries,
% a very reasonable result is obtained for moste of the European countries
%
% The major countires Spain, France, Germany, Poland are simulated very
% well, also many smaller countries, Benelux, Scandinavia. 
%
% (1) Failure Iceland: GLUES simulates a population which was not there
% (2) Failure Balkan: All of the balkan regions are overestimated with
% GLUES
% 
% This is a very promising result, iceland can be easily explained, so we
% have to deal with the balkan.  If at all, GLUES should underestimate the
% populatins there during bronze age, it is not, however.  This needs to be
% resolved.

neurope=length(europe)
lkk=zeros(neurope,length(ntime));

for i=1:neurope
  j=find(europe(i)==regnum);
